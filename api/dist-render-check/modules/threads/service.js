import { ThreadItemType, UnreadState, VoiceRegistrationState } from "@prisma/client";
import { prisma } from "../../lib/prisma.js";
import { decodeCursor, encodeCursor } from "../../lib/cursor.js";
import { AppError } from "../../lib/errors.js";
import { emitToBusiness } from "../../lib/socket.js";
function voicemailPreview(transcriptText) {
    return transcriptText?.trim() ? transcriptText.trim().slice(0, 80) : "New voicemail";
}
export async function findContactForNumber(businessId, e164) {
...
export async function recomputeThread(threadId) {
...
export async function syncNotificationStateForBusiness(businessId) {
...
function buildCursorWhere(fieldName, cursor) {
    if (!cursor)
        return undefined;
    return {
        OR: [
...
export async function getThreadById(businessId, threadId) {
...
export async function listThreadItems(input) {
    ...
    return {
        items: page.map((item) => ({
            ...
            payload: item.payloadRefType === "VOICEMAIL"
...
export async function listMailbox(input) {
    const limit = Math.min(Math.max(input.limit ?? 25, 1), 100);
    const cursor = decodeCursor(input.cursor);
    const items = await prisma.threadItem.findMany({
        where: {
    ...
    return {
        items: page.map((item) => {
            ...
            return {
                ...
                transcriptSnippet: voicemail?.transcriptStatus === "COMPLETED"
...
export async function markVoicemailHeard(voicemailId, businessId) {
