...
export async function resolveBusinessPhoneByIncomingNumber(toNumber) {
    if (!toNumber)
        return null;
...
export async function handleSmsStatus(payload) {
    const messageSid = payload.MessageSid?.trim();
    if (!messageSid) {
    ...
    await prisma.message.update({
        ...
        data: {
            ...
            deliveryStatus: payload.MessageStatus === "delivered"
                ? MessageDeliveryStatus.DELIVERED
                : payload.MessageStatus === "failed" || payload.MessageStatus === "undelivered"
                    ? MessageDeliveryStatus.FAILED
                    : MessageDeliveryStatus.SENT,
            errorCode: payload.MessageStatus === "failed" || payload.MessageStatus === "undelivered" ? "SMS_SEND_ERROR" : null,
        },
    });
}
export async function handleVoiceStatus(payload) {
    ...
    const eventType = dialCallStatus === "busy" || dialCallStatus === "no-answer" || dialCallStatus === "failed" || callStatus === "busy"
        ? "MISSED_CALL"
    ...
    await projectThreadItem({
        ...
        previewText: eventType === "MISSED_CALL"
            ? "Missed call"
...
export async function handleVoiceRecording(payload) {
    const businessId = payload.businessId ?? payload.BusinessId;
    const phoneNumberId = payload.phoneNumberId ?? payload.PhoneNumberId;
    const externalParticipantE164 = optionalE164(payload.externalParticipantE164 ?? payload.ExternalParticipantE164 ?? payload.From ?? payload.To);
...
export async function handleVoiceTranscription(payload) {
    const businessId = payload.businessId ?? payload.BusinessId;
