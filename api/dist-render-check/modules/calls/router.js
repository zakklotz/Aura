...
callsRouter.get("/access-token", requireUser, requireBusiness, async (req, res) => {
    try {
        ...
        sendAppError(res, error instanceof Error
...
callsRouter.post("/outbound", requireUser, requireBusiness, async (req, res) => {
    try {
        ...
        const phoneNumber = input.phoneNumberId != null ? { id: input.phoneNumberId } : await getPrimaryPhoneNumberForBusiness(viewer.businessId);
        if (!phoneNumber) {
